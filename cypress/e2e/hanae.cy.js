...
it("verifier le header", () => {
    ...
    cy.wait(3000)
   cy.get('[data-uuid="MJFtCCgVhXrVl7v9HA7EH_login"]').and('have.text', 'Log in')
   cy.wait(3000)
   cy.get('.Tabsstyles__Link-sc-1grh34k-0').and('have.text', 'Pricing')
   cy.get('.Buttonsstyles__PrimaryButton-sc-1jwidxo-1').and('have.text', 'Get Trello for free')
   cy.get('h1').and('have.text', 'Trello brings all your tasks, teammates, and tools together')
   cy.wait(3000)
});
   ...
   cy.get('[data-testid="card-back-due-date-button"]').click();
    ...
    cy.wait(5000);
    cy.get('[data-testid="card-back-labels-container"]').get('.color-blind-pattern-orange').should('be.visible');
    cy.get('.Y9J4BArcarEAX9 > .nch-icon > [data-testid="CloseIcon"]').click();
      });
